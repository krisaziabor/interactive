...
const generatePhotoPairs = (pairs) => {
  ...
  shuffledPairs.forEach(({ ad, art, title }, index) => {
    // Create ad photo element
    const adImg = document.createElement('img');
    adImg.src = ad;
    adImg.dataset.pairIndex = index;

    // Create art photo element
    const artImg = document.createElement('img');
    artImg.src = art;
    artImg.dataset.pairIndex = index;
    ...
    const handleHoverOut = () => {
      ...
      document.getElementById('hover-text').innerHTML = '<p>Hover over a photo to see more details.</p>';
    };
